...
ensure_whiff_db_pass() {
  ...
  # Migration: if an older secrets.env contains DB_PASS or PG_DB_PASS, we adopt it as WHIFF_DB_PASS once.
  ...
  # Migration path (older installs used DB_PASS/PG_DB_PASS)
  if [[ -z "$cur" || "$cur" == "CHANGE_ME" ]]; then
    cur="$(read_env_kv "$secrets" "DB_PASS")"
  fi
  if [[ -z "$cur" || "$cur" == "CHANGE_ME" ]]; then
    cur="$(read_env_kv "$secrets" "PG_DB_PASS")"
  fi
  ...
  # Keep secrets.env minimal: remove legacy keys if present.
  if [[ -f "$secrets" ]]; then
    sed -i -E '/^(DB_PASS|PG_DB_PASS)=/d' "$secrets" || true
  fi

  WHIFF_DB_PASS="$cur"
  # Back-compat: install_whiff.sh currently expects DB_PASS
  DB_PASS="$WHIFF_DB_PASS"
...
sync_whiff_into_wade_conf() {
  ...
  # Only bother with keys that actually have values set in this shell
  local k v
  for k in WHIFF_BIND WHIFF_PORT WHIFF_THREADS WHIFF_CTX; do
    v="${!k}"
    [[ -n "$v" ]] || continue
    _set_conf_kv "$conf" "$k" "$v"
  done
}
...
install -d /etc/whiff
cat >/etc/whiff/install.conf <<'WCONF'
# Autogenerated by WADE installer – used as --config for WHIFF install
if [ -f /etc/wade/wade.conf ]; then
  # shellcheck source=/etc/wade/wade.conf
  . /etc/wade/wade.conf
fi
WCONF
...
# Track whether we just created the primary WADE user and what password we used
LWADE_CREATED=0
LWADE_PASS=""
...
if ! id -u "$LWADEUSER" >/dev/null 2>&1; then
  ...
  if [[ "$NONINTERACTIVE" -eq 0 ]]; then
    while :; do
      read -s -p "Password for ${LWADEUSER}: " p1; echo
    ...
    echo "${LWADEUSER}:${LWADE_PASS}" | chpasswd || warn_note "useradd" "could not set password for ${LWADEUSER}"
  fi
...
# PostgreSQL role & database prompt (only if enabled)
PG_DB_USER="${LWADEUSER}"
PG_DB_NAME="${PG_DB_USER}"
PG_DB_PASS="${PG_DB_PASS:-}"

if [[ "${PG_CREATE_AUTOPSY_USER:-1}" == "1" ]]; then
  if [[ "$NONINTERACTIVE" -eq 0 ]]; then
    echo
    echo ">> PostgreSQL role/database"
    echo "   Role name : ${PG_DB_USER}"
    echo "   Database  : ${PG_DB_NAME}"

    # If we just created the Linux user and know their password, offer to reuse it
    if [[ "${LWADE_CREATED:-0}" -eq 1 && -n "${LWADE_PASS:-}" ]]; then
      read -r -p "Use SAME password for PostgreSQL role '${PG_DB_USER}' as Linux user '${LWADEUSER}'? [Y/n]: " ans
      ans="${ans:-Y}"
      if [[ "$ans" =~ ^[Yy]$ ]]; then
        PG_DB_PASS="$LWADE_PASS"
      fi
    fi

    # If we didn't reuse the Linux password (or user already existed), prompt for a DB password
    if [[ -z "${PG_DB_PASS:-}" ]]; then
      while :; do
        read -s -p "Password for PostgreSQL role '${PG_DB_USER}': " p1; echo
        read -s -p "Confirm: " p2; echo
        [[ "$p1" == "$p2" && -n "$p1" ]] && { PG_DB_PASS="$p1"; break; }
        echo "Mismatch/empty. Try again."
      done
    fi
  else
    # Noninteractive: auto-generate a DB password and log it
    if [[ -z "${PG_DB_PASS:-}" ]]; then
      PG_DB_PASS="$(tr -dc 'A-Za-z0-9@#%+=' </dev/urandom | head -c 24)"
      {
        echo "[wade] PostgreSQL password for role '${PG_DB_USER}' (auto-generated): ${PG_DB_PASS}"
        echo "[wade] Change it later with:"
        echo "       sudo -u postgres psql -c \"ALTER ROLE ${PG_DB_USER} WITH PASSWORD 'newpass';\""
      } >> "$LOG_FILE"
    fi
  fi
fi

export PG_DB_USER PG_DB_NAME PG_DB_PASS
...
# ===== WHIFF (local DFIR assistant / KB) prompts =====

# Defaults (from env or wade.conf-style values)
DEFAULT_WHIFF_ENABLE="${WHIFF_ENABLE:-1}"

DEFAULT_WHIFF_BIND="${WHIFF_BIND:-127.0.0.1}"
DEFAULT_WHIFF_PORT="${WHIFF_PORT:-8088}"
DEFAULT_WHIFF_THREADS="${WHIFF_THREADS:-8}"
DEFAULT_WHIFF_CTX="${WHIFF_CTX:-4096}"

DEFAULT_WHIFF_INSTALL_PG_LOCAL="${INSTALL_PG_LOCAL:-1}"
DEFAULT_WHIFF_DB_HOST="${DB_HOST:-127.0.0.1}"
DEFAULT_WHIFF_DB_PORT="${DB_PORT:-5432}"
DEFAULT_WHIFF_DB_NAME="${DB_NAME:-whiff}"
DEFAULT_WHIFF_DB_USER="${DB_USER:-whiff}"

DEFAULT_WHIFF_ONLINE_MODE="${ONLINE_MODE:-1}"
DEFAULT_WHIFF_DL_MODELS_NOW="${DL_MODELS_NOW:-1}"
DEFAULT_WHIFF_SEED_BASELINE_DOCS="${SEED_BASELINE_DOCS:-1}"
DEFAULT_WHIFF_RUN_INITIAL_CRAWL="${RUN_INITIAL_CRAWL:-0}"
DEFAULT_WHIFF_ENABLE_NIGHTLY="${ENABLE_NIGHTLY_BACKFILL:-1}"
DEFAULT_WHIFF_PATCH_WADE_ENV="${PATCH_WADE_ENV:-1}"
DEFAULT_WHIFF_PACKAGE_SPLUNK="${PACKAGE_SPLUNK_ADDON:-1}"

WHIFF_ENABLE_EFF="$DEFAULT_WHIFF_ENABLE"

if [[ "$NONINTERACTIVE" -eq 0 ]]; then
  echo
  echo ">> WHIFF (local DFIR assistant / knowledge base)"
  read -r -p "Install and configure WHIFF now? (y/N) [$( [[ "$DEFAULT_WHIFF_ENABLE" = "1" ]] && echo y || echo N )]: " __ans
  if [[ "${__ans:-}" =~ ^[Yy]$ ]]; then
    WHIFF_ENABLE_EFF="1"

    read -r -p "Whiff bind address [${DEFAULT_WHIFF_BIND}]: " tmp; WHIFF_BIND="${tmp:-$DEFAULT_WHIFF_BIND}"
    read -r -p "Whiff port [${DEFAULT_WHIFF_PORT}]: " tmp; WHIFF_PORT="${tmp:-$DEFAULT_WHIFF_PORT}"
    read -r -p "Model CPU threads [${DEFAULT_WHIFF_THREADS}]: " tmp; WHIFF_THREADS="${tmp:-$DEFAULT_WHIFF_THREADS}"
    read -r -p "Model context tokens [${DEFAULT_WHIFF_CTX}]: " tmp; WHIFF_CTX="${tmp:-$DEFAULT_WHIFF_CTX}"

    read -r -p "Install/manage local Postgres for WHIFF? (Y/n) [$( [[ "$DEFAULT_WHIFF_INSTALL_PG_LOCAL" = "1" ]] && echo Y || echo n )]: " tmp
    if [[ "${tmp:-}" =~ ^[Nn]$ ]]; then
      INSTALL_PG_LOCAL="0"
    else
      INSTALL_PG_LOCAL="1"
    ...
    read -r -p "Whiff DB host [${DEFAULT_WHIFF_DB_HOST}]: " tmp; DB_HOST="${tmp:-$DEFAULT_WHIFF_DB_HOST}"
    read -r -p "Whiff DB port [${DEFAULT_WHIFF_DB_PORT}]: " tmp; DB_PORT="${tmp:-$DEFAULT_WHIFF_DB_PORT}"
    ...
    if [[ "$ONLINE_MODE" -eq 1 ]]; then
      read -r -p "  Download models now? (Y/n) [$( [[ "$DEFAULT_WHIFF_DL_MODELS_NOW" = "1" ]] && echo Y || echo n )]: " tmp
      DL_MODELS_NOW="$( [[ "${tmp:-}" =~ ^[Nn]$ ]] && echo 0 || echo 1 )"

      read -r -p "  Seed baseline docs now? (Y/n) [$( [[ "$DEFAULT_WHIFF_SEED_BASELINE_DOCS" = "1" ]] && echo Y || echo n )]: " tmp
      SEED_BASELINE_DOCS="$( [[ "${tmp:-}" =~ ^[Nn]$ ]] && echo 0 || echo 1 )"

      read -r -p "  Run initial crawl after install? (y/N) [$( [[ "$DEFAULT_WHIFF_RUN_INITIAL_CRAWL" = "1" ]] && echo y || echo N )]: " tmp
      RUN_INITIAL_CRAWL="$( [[ "${tmp:-}" =~ ^[Yy]$ ]] && echo 1 || echo 0 )"
    else
      DL_MODELS_NOW=0
    ...
    read -r -p "Install nightly WHIFF backfill timer? (Y/n) [$( [[ "$DEFAULT_WHIFF_ENABLE_NIGHTLY" = "1" ]] && echo Y || echo n )]: " tmp
    ENABLE_NIGHTLY_BACKFILL="$( [[ "${tmp:-}" =~ ^[Nn]$ ]] && echo 0 || echo 1 )"

    read -r -p "Patch /etc/wade/wade.env with WHIFF_URL? (Y/n) [$( [[ "$DEFAULT_WHIFF_PATCH_WADE_ENV" = "1" ]] && echo Y || echo n )]: " tmp
    PATCH_WADE_ENV="$( [[ "${tmp:-}" =~ ^[Nn]$ ]] && echo 0 || echo 1 )"

    read -r -p "Package Splunk WHIFF add-on tarball? (Y/n) [$( [[ "$DEFAULT_WHIFF_PACKAGE_SPLUNK" = "1" ]] && echo Y || echo n )]: " tmp
    PACKAGE_SPLUNK_ADDON="$( [[ "${tmp:-}" =~ ^[Nn]$ ]] && echo 0 || echo 1 )"
  else
    WHIFF_ENABLE_EFF="0"
  fi
else
  # non-interactive -- trust env / wade.conf defaults
  WHIFF_ENABLE_EFF="${DEFAULT_WHIFF_ENABLE}"
  WHIFF_BIND="${DEFAULT_WHIFF_BIND}"
  WHIFF_PORT="${DEFAULT_WHIFF_PORT}"
  WHIFF_THREADS="${DEFAULT_WHIFF_THREADS}"
  WHIFF_CTX="${DEFAULT_WHIFF_CTX}"
  INSTALL_PG_LOCAL="${DEFAULT_WHIFF_INSTALL_PG_LOCAL}"
  DB_HOST="${DEFAULT_WHIFF_DB_HOST}"
  ...
  RUN_INITIAL_CRAWL="${DEFAULT_WHIFF_RUN_INITIAL_CRAWL}"
  ENABLE_NIGHTLY_BACKFILL="${DEFAULT_WHIFF_ENABLE_NIGHTLY}"
  PATCH_WADE_ENV="${DEFAULT_WHIFF_PATCH_WADE_ENV}"
  PACKAGE_SPLUNK_ADDON="${DEFAULT_WHIFF_PACKAGE_SPLUNK}"
fi
...
export WHIFF_NONINTERACTIVE=1 \
       ...
       ENABLE_NIGHTLY_BACKFILL PATCH_WADE_ENV PACKAGE_SPLUNK_ADDON
...
if [[ "$NONINTERACTIVE" -eq 0 ]]; then
  ...
  echo " WHIFF       : $( [[ "${WHIFF_ENABLE_EFF:-0}" = "1" ]] && echo enabled || echo disabled )  bind=${WHIFF_BIND:-$DEFAULT_WHIFF_BIND}:${WHIFF_PORT:-$DEFAULT_WHIFF_PORT}"
...
if [[ "${WHIFF_ENABLE_EFF:-0}" == "1" ]]; then
  ...
  # Make sure we have a non-empty DB_PASS when running non-interactive
  if [[ -z "${DB_PASS:-}" && -n "${PG_DB_PASS:-}" ]]; then
    DB_PASS="$PG_DB_PASS"
  fi
  ...
  if [[ -z "${DB_PASS:-}" ]]; then
    ...
    env \
      ...
      PIP_NO_INDEX=0 \
      WHIFF_NONINTERACTIVE=1 \
      INSTALL_PG_LOCAL="${INSTALL_PG_LOCAL:-1}" \
      ...
      WHIFF_THREADS="${WHIFF_THREADS:-8}" \
      WHIFF_CTX="${WHIFF_CTX:-4096}" \
      ONLINE_MODE="${ONLINE_MODE:-1}" \
      DL_MODELS_NOW="${DL_MODELS_NOW:-1}" \
      SEED_BASELINE_DOCS="${SEED_BASELINE_DOCS:-1}" \
      RUN_INITIAL_CRAWL="${RUN_INITIAL_CRAWL:-0}" \
      ENABLE_NIGHTLY_BACKFILL="${ENABLE_NIGHTLY_BACKFILL:-1}" \
      PATCH_WADE_ENV="${PATCH_WADE_ENV:-1}" \
      PACKAGE_SPLUNK_ADDON="${PACKAGE_SPLUNK_ADDON:-1}" \
      bash "${SCRIPT_DIR}/WHIFF/install_whiff.sh" \
        --config /etc/whiff/install.conf \
        --non-interactive \
      || warn_note "whiff" "install reported issues"
  fi
else
