...
1.  Defaults in code (`DEFAULT_MATRIX` in `tool_routing.py`)
2.  YAML `routing.defaults`
3.  YAML `os_overrides` (based on detected OS family)
4.  YAML `location_overrides` (based on staging location)
5.  Environment variables (`WADE_ROUTE_<CLASS>_<PROFILE>`)
